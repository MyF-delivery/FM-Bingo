...
function checkBingoPatterns(marked) {
    ...
    if (cornerWin) patterns.push({ type: 'corner', index: 0, label: 'ማዕዘን' });
...
function startGame(stake, playerCards) {
    ...
    const interval = setInterval(() => {
        ...
        if (callIndex >= allNumbers.length || gameState.currentWinner) {
            clearInterval(interval);
            endGame();
            return;
        }
        ...
        if (winners.length > 0) {
            gameState.currentWinner = winners;
            clearInterval(interval);
            
            const totalPrize = stake * gameState.totalCardsInPlay * 0.7;
            const prizePerWinner = Math.round(totalPrize / winners.length);
